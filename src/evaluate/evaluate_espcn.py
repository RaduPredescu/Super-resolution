...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate ESPCN.")
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(dataset)), desc="Evaluating ESPCN"):
            sample = dataset[idx]
            ...
            espcn_np = pred.permute(1, 2, 0).numpy()
            hr_np = hr.permute(1, 2, 0).numpy()

            psnr = calculate_psnr(espcn_np, hr_np, crop=args.scale)
            ssim = calculate_ssim(espcn_np, hr_np, crop=args.scale)
            ...
            if idx < args.save_examples:
                ...
                espcn_pil = tensor_to_pil(pred)

                lr_pil = tensor_to_pil(sample["lr"])
                ...
                comparison.save(
                    examples_dir / f"{safe_filename(name)}_bicubic_espcn_hr.png"
                )
    ...
    output_csv = output_dir / f"metrics_espcn_{args.dataset}_{checkpoint_name}.csv"
    summary_path = output_dir / f"summary_espcn_{args.dataset}_{checkpoint_name}.txt"
