...
class SRCNNREDSDataset(Dataset):
    """
    Dataset pentru antrenarea / fine-tuning SRCNN pe REDS.

    REDS expected structure:
    ...
    SRCNN este frame-by-frame:
        input  = LR frame upscaled bicubic la dimensiunea HR
        target = HR frame

    Pentru train:
        - alegem un crop HR random
        - alegem crop-ul LR corespondent
        - upscalăm LR crop la dimensiunea HR crop
    """
    ...
    def __init__(
        ...
        self.samples = self._collect_frame_pairs()
    ...
    def _collect_frame_pairs(self) -> list[tuple[Path, Path, str]]:
    ...
    def _random_aligned_crop(
        ...
        # Aliniere cu factorul de scalare, ca să corespundă exact crop-ul LR.
    ...
    def __getitem__(self, idx: int) -> dict:
        hr_path, lr_path, rel_name = self.samples[idx]
        ...
        # SRCNN primește LR upscaled bicubic la dimensiunea HR.
        bicubic = lr.resize(hr.size, Image.Resampling.BICUBIC)

        return {
            "input": pil_to_tensor(bicubic),
...
def load_pretrained_if_needed(
    ...
    """
    Pentru transfer learning / fine-tuning.

    Exemplu:
        pretrained = checkpoint SRCNN antrenat pe Vimeo-90K
        apoi continuăm antrenarea pe REDS
    """
...
def write_config_to_tensorboard(
    ...
    config_lines = [
        "# SRCNN REDS training configuration",
        "",
        f"device: {device}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train / fine-tune SRCNN on REDS.")
    ...
    train_dataset = SRCNNREDSDataset(
        root=args.root,
        split="train",
        scale=args.scale,
    ...
    # Pentru REDS, validarea pe frame-uri întregi poate consuma mult VRAM,
    # deci folosim batch_size=1 la validare.
