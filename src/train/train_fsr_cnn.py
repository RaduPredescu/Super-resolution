...
class FSRCNNVimeoDataset(Dataset):
    def __init__(
        self,
        root: str,
        split: str,
...
class FSRCNNREDSDataset(Dataset):
    def __init__(
        self,
        root: str,
        split: str,
...
def build_datasets(args: argparse.Namespace) -> tuple:
    if args.dataset == "vimeo":
        train_dataset = FSRCNNVimeoDataset(
            root=args.root,
            split="train",
            scale=args.scale,
        ...
        val_dataset = FSRCNNVimeoDataset(
            root=args.root,
            split="test",
            scale=args.scale,
            crop_size=args.crop_size,
    ...
    if args.dataset == "reds":
        train_dataset = FSRCNNREDSDataset(
            root=args.root,
            split="train",
            scale=args.scale,
            crop_size=args.crop_size,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train / fine-tune FSRCNN.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional FSRCNN checkpoint for transfer learning / fine-tuning.",
    )
