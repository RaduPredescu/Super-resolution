...
class SRCNNVimeoDataset(Dataset):
    """
    Dataset pentru antrenarea SRCNN pe Vimeo-90K.

    SRCNN este frame-by-frame:
        input  = LR im4 upscaled bicubic la dimensiunea HR
        target = HR im4

    Pentru train folosim random crop din HR, apoi generăm LR din acel crop.
    """
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # LR -> bicubic-upscaled input pentru SRCNN
        bicubic = lr.resize((w, h), Image.Resampling.BICUBIC)
...
def sanitize_checkpoint_name(name: str) -> str:
    """
    Transformă numele introdus de utilizator într-un nume valid de fișier.
    """
    name = name.strip()
...
def load_pretrained_if_needed(
    ...
    """
    Opțional pentru transfer learning / fine-tuning.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train SRCNN on Vimeo-90K.")
    ...
    train_dataset = SRCNNVimeoDataset(
        root=args.root,
        split="train",
        scale=args.scale,
